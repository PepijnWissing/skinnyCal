...
- command (dict; optional):
    ...
    Each type  maps 1:1 to a real FullCalendar API method. Supported:
    {'type': 'next' | 'prev' | 'today'} to navigate;  {'type':
    'changeView', 'view': '<name>'} to switch view;  {'type':
...
- contextMenu (boolean | number | string | dict | list; optional):
    JSON-serializable description of the most recent recognized
    right-click,  for use in Dash callbacks. Contains `target`
    (\"event\" | \"date\" |  \"resource\", precedence in that order),
    `calendarId`, `viewType`, the  resolved `date` ({start, allDay,
    timeZone}), `resource`  ({id, title, extendedProps}), `event`
    (snapshot incl. `resourceIds`),  `jsEvent` (pointer coordinates +
    modifier keys), and a monotonically  increasing `sequence` so
    repeated right-clicks at the same location still  produce a
    distinct value. All top-level keys are always present; anything
    unavailable is None. Typed `any` (not `func`) so
    dash-generate-components exposes it as a Dash prop. See README for
    the  slot-start / timezone caveats.

- contextMenuEnabled (boolean; default False):
    ...
    Disabled by default. When False no listener is installed and the
    browser  menu behaves normally everywhere. When True,
    right-clicking a recognized  calendar target emits `contextMenu`
    and suppresses the browser menu for  that target only;
    right-clicks on toolbar controls or other unrecognized  chrome
    keep the normal browser menu. skinnycal emits the context only —
...
- eventClassNames (list | dict | string; optional):
    Class names applied to every event block. Composed with — not
    replaced by —  the per-event `extendedProps.classNames` channel:
    any event dict carrying  `{\"extendedProps\": {\"classNames\":
    [\"is-conflict-hard\", ...]}}` gets those  classes on its
    `.fc-event` root, and FullCalendar re-evaluates them when  the
    event's data changes, so Dash can toggle per-block state by
    updating  only the affected events. (FullCalendar's native
    top-level `classNames`  key on an event object also still works
    and is merged in by FullCalendar.)  See FullCalendar docs.
...
- eventDataAttributes (list of strings; optional):
    List of `extendedProps` keys to mirror onto each event block's
    `.fc-event`  root as `data-*` attributes, so wrapper JS can read
    event data off the DOM  instead of FullCalendar internals. Keys
    are kebab-cased  (`courseCode` -> `data-course-code`, i.e.
    `el.dataset.courseCode`);  `None`/missing values are skipped and
    objects/arrays are JSON-encoded.    Every block additionally
    always gets `data-event-id=\"<event.id>\"` (for  events that have
...
- eventMouseEnter (boolean | number | string | dict | list; optional):
    Object describing the event the pointer entered (id, title, start,
    end,  extendedProps, jsEvent.pageX/pageY), for use in Dash
    callbacks. Typed  `any` (not `func`) so dash-generate-components
    exposes it as a Dash prop.

- eventMouseLeave (boolean | number | string | dict | list; optional):
    Object ({id}) describing the event the pointer left, for use in
    Dash  callbacks. Typed `any` (not `func`) so it is exposed as a
    Dash prop.
...
- resources (list | string; optional):
    Array of resource objects used by Scheduler (Premium) resource
    views  (e.g. one lane per trainer), or a URL/function supplying
    them. Forwarded  verbatim to FullCalendar. See FullCalendar docs.

- schedulerLicenseKey (string; optional):
    FullCalendar Premium (Scheduler) license key. Required when using
    any Premium plugin  such as resource views or scrollgrid. See
...
- windowResizeDelay (number; optional):
    ...
    def __init__(
        ...
        resources: typing.Optional[typing.Union[typing.Sequence, str, typing.Any]] = None,
        ...
        eventDataAttributes: typing.Optional[typing.Sequence[str]] = None,
        ...
        contextMenuEnabled: typing.Optional[bool] = None,
