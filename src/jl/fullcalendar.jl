...
- `command` (Dict; optional): An object specifying a command to execute on the calendar API. Each type
...
- `eventMouseEnter` (Bool | Real | String | Dict | Array; optional): Object describing the event the pointer entered (id, title, start, end,
...
- `eventMouseLeave` (Bool | Real | String | Dict | Array; optional): Object ({id}) describing the event the pointer left, for use in Dash
...
- `resources` (Array | String; optional): Array of resource objects used by Scheduler (Premium) resource views
